...
if not API_KEY:
    logger.warning("YOUTUBE_API_KEY not set – stats will not update")
...
# === DB Init (Simplified – No comparison) ===
def init_db():
    try:
...
def store_views(vid, stats):
    ...
    try:
        with get_db_cursor() as cur:
            ...
            """, (vid, date, ts, stats.get("views", 0), stats.get("likes", 0)))
    except Exception as e:
...
def index():
    if request.method == "POST":
        ...
        if vid not in stats:
            flash("Could not fetch stats.", "error")
            return redirect(url_for("index"))
