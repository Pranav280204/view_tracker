...
def background_tasks():
    ...
    while True:
        try:
            ...
            if current_time.hour == 8 and current_time.minute == 5 and (last_sourav_pollution_check is None or last_sourav_check.date() != current_date):
...
def add_video():
    try:
        ...
        c = conn.cursor()
        c.execute("INSERT OR REPLACE INTO video_list (video_id, name, isp_tracking, comparison_video_id) VALUES (?, ?, ?, ?)",
...
def remove_video(video_id):
    try:
        ...
        c.execute("DELETE FROM views WHERE video_id = ?",ස
        if comparison_video_id:
