...
# PostgreSQL (Render) — with sslmode=prefer
POSTGRES_URL = (
...
def init_db():
    try:
        ...
        logger.warning(f"DB init skipped (likely exists): {e}")
...
def store_views(video_id, stats):
    ...
    try:
        ...
        logger.error(f"Failed to store views for {video_id}: {e}")
...
def background_tasks():
    while True:
        try:
            ...
            time.sleep(10)  # Avoid tight loop on failure
...
def index():
    ...
    try:
        ...
        init_db()
        return render_template("index.html", videos=[], error_message="Database error. Retrying...")
...
def add_video():
    try:
        ...
        flash("Failed to add video. Please try again.", "error")
        return redirect(url_for("index"))
...
def stop_tracking(video_id):
    try:
        ...
        flash("Failed to stop tracking.", "error")
    return redirect(url_for("index"))
...
def remove_video(video_id):
    try:
        ...
        flash("Failed to remove video.", "error")
    return redirect(url_for("index"))
